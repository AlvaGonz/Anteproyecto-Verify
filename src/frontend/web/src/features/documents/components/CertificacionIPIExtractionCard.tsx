...
export const CertificacionIPIExtractionCard: React.FC<CertificacionIPIExtractionCardProps> = ({ extraction }) => {
  ...
  if (isProcessing) {
    return (
      <div className="w-full mt-2 p-4 rounded-xl border border-primary/20 bg-primary/5 flex flex-col items-center justify-center gap-3 animate-pulse" data-testid="certificacion-ipi-extraction-card">
  ...
  if (isError) {
    return (
      <div className="w-full mt-2 p-4 rounded-xl border border-error/20 bg-error/5 flex items-start gap-3" data-testid="certificacion-ipi-extraction-card">
    ...
    <div className="w-full mt-2 p-4 sm:p-5 rounded-xl border border-border/50 bg-surface-container-low shadow-sm" data-testid="certificacion-ipi-extraction-card">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-sm font-bold text-secondary tracking-tight">Extracción de Certificación IPI</h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {renderField("No. de Certificación", extraction.numeroCertificacion, true, "field-numeroCertificacion")}
        {renderField("No. Inmueble", extraction.numeroInmueble, true, "field-numeroInmueble")}
        {renderField("Parcela No.", extraction.parcelaNumero, true, "field-parcelaNumero")}
      </div>
