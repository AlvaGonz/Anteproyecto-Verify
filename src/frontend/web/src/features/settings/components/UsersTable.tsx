...
import { 
  Mail, Phone, Layers, Pencil, Trash2, Plus, Shield, Search
} from "lucide-react";
...
export const UsersTable: React.FC<UsersTableProps> = ({ users, plans, onEdit, onDelete, onAddNew }) => {
  ...
  const sortedPlans = useMemo(() => {
    return [...plans]
      .filter(p => p.price > 0 && p.name !== "Gratuito" && p.name !== "Consultation")
  ...
  const filteredUsers = useMemo(() => {
    if (!searchQuery.trim()) return currentTabUsers;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-border shadow-sm flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <button type="button" onClick={onAddNew} className="vf-btn-primary flex items-center gap-2 text-sm px-4 py-2 shrink-0">
