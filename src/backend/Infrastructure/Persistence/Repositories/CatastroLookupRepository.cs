...
public class CatastroLookupRepository : ICatastroLookupRepository
{
    ...
    public async Task<List<CatastroLookupDto>> GetByMatriculaOrDesignacionAsync(string? matricula, string? designacion, CancellationToken cancellationToken = default)
    {
        var connection = _context.Database.GetDbConnection();
        var command = connection.CreateCommand();
        command.CommandText = @"
            ...
            FROM CatastroTitulo c
            LEFT JOIN PagoIPI p ON c.Rnc = p.Rnc
            WHERE (@p0 IS NOT NULL AND c.Matricula = @p0)
               OR (@p1 IS NOT NULL AND c.CodigoDesignacionCatastral = @p1)
        ";

        var p0 = command.CreateParameter();
        p0.ParameterName = "@p0";
        p0.Value = string.IsNullOrWhiteSpace(matricula) ? DBNull.Value : (object)matricula;
        command.Parameters.Add(p0);

        var p1 = command.CreateParameter();
        p1.ParameterName = "@p1";
        p1.Value = string.IsNullOrWhiteSpace(designacion) ? DBNull.Value : (object)designacion;
        command.Parameters.Add(p1);

        bool wasOpen = connection.State == ConnectionState.Open;
        if (!wasOpen) await connection.OpenAsync(cancellationToken);

        var results = new List<CatastroLookupDto>();
        try
        {
            using var reader = await command.ExecuteReaderAsync(cancellationToken);
            while (await reader.ReadAsync(cancellationToken))
            {
                results.Add(new CatastroLookupDto(
                    reader.IsDBNull(0) ? string.Empty : reader.GetString(0),
                    reader.IsDBNull(1) ? string.Empty : reader.GetString(1),
                    reader.IsDBNull(2) ? 0 : reader.GetDecimal(2),
                    reader.IsDBNull(3) ? null : reader.GetString(3),
