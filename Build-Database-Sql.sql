...
GO
                ...
                VALUES 
                ('6316a69c-0249-4547-bc2e-6779e2fb41e9', 'CREADO', 'Creado', 'Proyecto recién creado', 'El proyecto ha sido registrado en la plataforma.', '#9BACD8', 1, '2026-08-01 02:26:33'),
                ('64da441f-8ace-4ff0-8347-8a38f9b1f40a', 'EDITADO', 'Editado', 'Proyecto editado por el usuario', 'El proyecto ha sufrido modificaciones.', '#F98513', 1, '2026-08-01 02:26:33'),
                ('18c7a6ef-589f-49e3-b162-90b2aa8e4e06', 'REVISION', 'En Revisión', 'El proyecto está siendo revisado', 'Se están verificando los documentos y datos.', '#EAB308', 1, '2026-08-01 02:26:33'),
                ('3d9b7f13-5d80-41fa-8208-2ec616ef7778', 'OBSERVACION', 'Con Observación', 'El proyecto requiere atención', 'Se encontraron observaciones que deben corregirse.', '#EF4444', 1, '2026-08-01 02:26:33'),
                ('0dd5695a-5f19-4446-aa4b-d128d3bc90a7', 'PUBLICADO', 'Publicado', 'Proyecto validado y publicado', 'El proyecto ha completado su proceso y está publicado.', '#10B981', 1, '2026-08-01 02:26:33');
GO

ALTER TABLE [ProyectosInmobiliarios] ADD [EstadoId] uniqueidentifier NOT NULL DEFAULT '6316a69c-0249-4547-bc2e-6779e2fb41e9';
GO

                UPDATE ProyectosInmobiliarios SET EstadoId = '6316a69c-0249-4547-bc2e-6779e2fb41e9';
GO
...
ALTER TABLE [Usuario] ADD [RecoveryCodesHashJson] nvarchar(2000) NULL;
GO

ALTER TABLE [Usuario] ADD [TwoFactorEnabled] bit NOT NULL DEFAULT CAST(0 AS bit);
GO

ALTER TABLE [Usuario] ADD [TwoFactorSecretEncrypted] nvarchar(500) NULL;
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[DGII]') AND [c].[name] = N'Rnc');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [DGII] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [DGII] ALTER COLUMN [Rnc] varchar(20) NOT NULL;
GO
