...
0. **Initialize Planning Files** (@planning-with-files)
   ...
   - Populate with workflow ID `restructure-backend`, objectives, and checklist.
...
5. **Verify `server/src/shared/`** contains only truly shared utilities (no domain-specific logic).
   - GATE: `grep -r "from.*modules/" server/src/shared/` returns 0 matches. If FAIL → stop and report. Do NOT proceed. MAX_RETRIES: 3.
...
8. **Commit**: `refactor(backend): migrate to server/src/modules/<domain>/`
