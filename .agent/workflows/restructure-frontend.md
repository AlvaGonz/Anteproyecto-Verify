...
0. **Initialize Planning Files** (@planning-with-files)
   ...
   - Populate with workflow ID `restructure-frontend`, objectives, and checklist.
...
7. **Run full frontend validation**: `npm run lint && npx tsc --noEmit && npx vitest run`
   - GATE: All exit 0. If FAIL → stop and report. Do NOT proceed. MAX_RETRIES: 3.

8. **Commit**: `refactor(frontend): migrate to src/features/<domain>/`
