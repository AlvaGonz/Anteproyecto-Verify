...
0. **Initialize Planning Files** (@planning-with-files)
   ...
   - Populate with the workflow ID `cleanup-dead-code`, current objectives, and checklist before touching any project files.
...
3. **Remove confirmed dead files**: delete only files confirmed unused in step 1 and 2.
   - GATE: `git status` shows only expected deletions. If FAIL → stop and report. Do NOT proceed. MAX_RETRIES: 3.
