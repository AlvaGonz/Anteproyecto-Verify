...
- **Diseño (clean architecture, sin hardcodeo en UI)**: la resolución vive UNA vez en `Domain/Policies/PublicIdentityResolver.cs` (`PublicPresentation` VO con fallbacks: nickname faltante → nombre real; RNC faltante → cédula; razón social faltante → línea oculta; sin preferencia → datos registrales del proyecto, comportamiento heredado). `ProjectService.MapToDto` consume el resolver vía assembler `ToPresentationDto` → contrato explícito `ProjectRegistrantPublicPresentationDto` (`PresentacionPublica`), la UI solo renderiza campos resueltos (legacy branch solo si el payload no trae `presentacionPublica`).
- **Persistencia**: enums `NombrePublicoModo`/`IdentificacionPublicaModo` (Domain/Enums) como string en `Usuario` (HasConversion<string>), método `Usuario.UpdatePreferenciasPublicas`. Migración `20260803210150_AddPublicDisplayPreferencesToUsuario` (2 columnas nvarchar(20) nullable) — aplicada manualmente a la DB docker (`verifinca-spm-uce-2026`) vía SQL + `__EFMigrationsHistory` (precedente: 3e13ae82). ⚠️ `appsettings.Development.json` apunta a `localhost\SQLEXPRESS\VeriFincaDb` (baseline rota); la DB real es el contenedor docker.
- **API**: `PATCH /api/auth/preferences` (`UpdatePublicPreferencesCommand/Handler`, wire: `realName|nickname` + `cedula|rnc`, 400 en valores inválidos); `GET /api/auth/me` expone ambos modos para hidratar el tab.
- **Tests**: `PublicIdentityResolverTests` 9/9 (teorías + fallbacks). E2E `e2e/projects/settings-preferences.spec.ts` 7/7 (tab visible, nickname+cédula, real+cédula, nickname+RNC, real+RNC, fallbacks, regresión de tabs) con route-mocks que replican el contrato del resolver. TDD red→green→refactor.
- **Reparaciones pre-existentes** (proyectos de test no compilaban desde 5372dbbd): fakes `IAuditLogger.Append` (2), constructores `ProjectService` (3) + `AuthController` (handler nuevo) en UnitTests y Api.Tests. UnitTests: 374/8 (mismos 8 fallos pre-existentes documentados; baseline no compila), Api.Tests 30/2 (2 ambientales: `EmailOtpLastSentUtc` duplicado en bootstrap contra SQLEXPRESS local).
- **Gate Playwright completo**: 190 passed / 7 failed — los 7 fallos (OCR, resend-verifier, create-project, public-directory-filter, pending-plan-redirect, pricing-checkout flaky) son PRE-EXISTENTES: verificados con checkout del frontend baseline (e385dc9e) → 9 failed. Cero regresiones; el spec nuevo pasa 7/7.
- **Fix UX posterior**: tab movido a la derecha de "Mi Perfil" + restaurado wiring de `Palette`/`PreferenciasSection`/`TabId` (una edición externa parcial lo había dejado roto con `ReferenceError: Palette is not defined` — hard refresh del navegador lo aclara).
- Status: COMPLETE. Commits: 19ebb3d7 (RED), b2f4602c (frontend GREEN), 005f3839 (backend+policy+test repairs), b95574bb (fix posicion/wiring).
...
# Progress: Seed de documentos MOC (test_docs) para 10 proyectos + Torre Playa Dorada Beach (2026-08-03, seed-documents)

- **Feature**: los 10 proyectos del seeder + "Torre Playa Dorada Beach" tienen los 6 documentos MOC de `test_docs/` (Título, Estado Jurídico, Mensura, Cédula, Certificación IPI, Poder Notarial), visibles end-to-end en `/#/admin/projects/:id/validations` (RequiredDocumentsList).
- **Storage**: los archivos se COPIAN al blob storage real (`IBlobStorageService`, Azurite localhost:10000) en seed time; `Documento.RutaArchivo` guarda la URL devuelta (fallback a URL mock si el blob no está disponible, mismo patrón que el seeder existente). Referenciar el filesystem era imposible: el endpoint de descarga lee de blob.
- **Seeder**: `SeedTestDocumentsAsync` en `AppDbContextSeeder.cs`, corre DESPUÉS de los proyectos (dependencia de orden), dev-only (SeedAsync solo corre en Development/UseMockData). Ruta fuente configurable `Seed:TestDocumentsPath` (default relativo `test_docs`, git-tracked, sin rutas absolutas de Windows). Sanitización: `Path.GetFileName` + allowlist de 6 carpetas fijas (sin path traversal).
- **Idempotencia**: clave (ProyectoId, TipoDocumento) — no hay unique constraint en `Documentos` (solo FK), el chequeo es en código. Verificado: 2º run = 0 insertados / 66 existentes.
- **Mapping**: carpeta→DocumentType canónico extraído a `TestDocumentFolders` (Cedula→ID=4, Certificación IPI→8, Estado Juridico→22, Planos de Mensura→24, Poder Notarial→11, Título de Propiedad→21). Cédula=ID(4) a propósito: RequiredDocumentsList matchea exacto (sin aliasing canónico).
- **Tests**: TDD red→green→refactor. `e2e/projects/seed-documents.spec.ts` 11/11 (10 proyectos + Torre), login fresco por test (el backend rota el refreshToken en cada /auth/refresh — un storageState compartido se invalidaba entre tests), assertions web-first sobre `requirement-row-*` (sin botón "Subir" = subido). Commits: 3888de7e (red), a69f02b1 (green), 803b6685 (refactor).
- **Gate Playwright completo**: 175 passed / 28 failed — 27 fallos PRE-EXISTENTES (settings-users-crud, subscription-plan-enforcement, guest-subscription-display, create-project, pricing-checkout, public-directory-filter, resend-failure-verifier): verificados en aislamiento, specs con route-mocks que no leen Documentos, API corriendo con el binario pre-cambio; el entry anterior de progress.md ya documentaba estos specs rotos. Mi spec: 11/11 estable bajo carga paralela (fix: timeout 20s para el gate de 3 APIs de la página de validación).
- Status: COMPLETE.

# Progress: Code quality cleanup del working tree (2026-08-03, cleanup)

- **Scope**: archivos modificados sin commitear (backend C#, frontend TS/TSX, specs e2e) — sin tocar archivos ajenos al diff.
- **Fix indentation**: `CertificadoTituloExtractionCard.tsx:137` — `loadMunicipalities()` con 6 espacios vs 4 del bloque (única llamada realmente mal indentada; PlanoMensura/EstadoJuridico verificados correctos a su nivel de anidación).
- **Debug code removed**: `pricing-checkout.spec.ts` — eliminado `console.log('API CALLS (failing run)')` + toda la instrumentación `apiCalls` (request/response/requestfailed handlers, 20 líneas muertas) + doble blank line. `create-project.spec.ts` ya había removido su `console.log` en el diff previo.
- **Verificado sin cambios necesarios**: `geo.ts` (nuevo, limpio), `SubscriptionService.cs` (exceptions tipadas `NotFoundException`/`BadRequestException` existen), `DocumentService.cs`/`ProjectService.cs` (constructores `Auditoria` 9-arg y `IAuditLogger.Append` validados contra la interfaz), uso de `fetchMunicipalities` en las 3 cards.
- **Gates**: `dotnet build` Api + UnitTests 0 warnings/0 errors; tests `SubscriptionServiceExceptionTests` 10/10; frontend `tsc -b` clean; eslint sobre archivos modificados sin errores NUEVOS (los 14 reportados son pre-existentes); tsc root clean (solo errores pre-existentes en `projectsApi.unit.test.ts`).
- **Flagged (manual, no tocado)**: `tsconfig.tsbuildinfo` commiteado en git (build artifact); 26 `console.log` de debug en specs e2e NO modificadas (project-crud, dashboard, ocr-cedula, certificacion-ipi-ocr, project-search, project-reports, route-performance); 10 disable-comments `react-doctor/*` referenciando plugin no instalado en eslint.config.js (useDocuments.ts, useAuth.ts, useReports.ts, RegisterForm.tsx, RulesManagePage.tsx); 10 errores/warnings eslint pre-existentes en las 3 cards (set-state-in-effect, exhaustive-deps) y `no-explicit-any`/`no-useless-assignment` en useProjects.ts/useDocuments.ts — requieren refactor de efectos React, fuera del alcance cleanup.
- Status: COMPLETE (sin commits; cambios de cleanup sin commitear).

# Progress: Test del hook de indexación codebase-memory-mcp (2026-08-03, cbm-hook)

- **Objetivo**: probar `.git/hooks/post-commit` (re-index automático del grafo en cada commit). Gate del usuario: nodes/edges deben aumentar.
- **Diagnóstico (root cause doble)**: (1) La DB del proyecto (junio, versión anterior) tenía schema drift: el binario 0.9.0 espera columnas nuevas (ej. `nodes.snippet` no existe en la DB vieja). El path de extracción crasheaba (worker exit 1, logs de 0 bytes) mientras el path de skip quedaba limpio -> índice congelado en 19926 nodes desde hace días, geo.ts sin indexar, fetchMunicipalities viejo (component-local) todavía en el grafo. (2) El hook usaba `command -v` sobre ruta absoluta + `&` sin nohup/redirect: git esperaba los pipes del hijo y el proceso hijo moría al salir el hook.
- **Aislamiento del crash**: bisect por eliminación (moví src/, tests/, root files, .git a temp) -> el crash persistía con el árbol casi vacío; tinytest (repo limpio, DB fresca) indexó bien; `--name CBM-BISECT-TEST` (DB nueva, mismo árbol) indexó bien -> crash = estado de la DB, no un archivo.
- **Fix**: delete_project (hubo que matar los 2 procesos MCP de la sesión 32468/34552 que tenían la DB lockeada; el de Antigravity IDE 3756 no bloquea) + rebuild `--mode full` -> 15852 nodes / 34733 edges, internally consistent (expected == stored). Grafo verificado: fetchMunicipalities -> api/geo.ts, loadMunicipalities -> las 3 cards (líneas actuales), StatusHistory.tsx indexado.
- **Hook arreglado** (`.git/hooks/post-commit`): `[ -x "$CBM_BIN" ]` + `nohup ... >/dev/null 2>&1 &` + `exit 0`.
- **Prueba end-to-end**: edición transitoria en geo.ts (hookProbe) -> `sh .git/hooks/post-commit` -> hookProbe apareció en search_graph; revert -> 15852/34733 restaurados. Incremental funciona en ambas direcciones.
- **Delta 19926->15852**: la DB vieja fue construida con config/versión distinta; el grafo nuevo es consistente y refleja el árbol actual.
- **Nota**: los MCP tools de grafo de ESTA sesión quedaron muertos (procesos matados, no auto-restart); el CLI funciona standalone. Reiniciar opencode para revivir las tools MCP. Working tree intacto (solo 3 specs e2e modificadas pre-existentes). Temp de bisect en `C:\Users\Alva\AppData\Local\Temp\opencode\bisect1` (copias, borrable).
- Status: COMPLETE.
