...
// Scans .agents/skills/ for SKILL.md files, validates schema, sanitizes content
...
export function scanSkills() {
  ...
  if (!existsSync(SKILLS_DIR)) {
    console.warn(`[skill-scanner] Skills directory not found: ${SKILLS_DIR}`);
    return [];
  }
  ...
  for (const dir of skillDirs) {
    ...
    const skillFilePath = join(SKILLS_DIR, dir, 'SKILL.md');
    
    if (!existsSync(skillFilePath)) {
      console.warn(`[skill-scanner] No SKILL.md in ${dir} — skipping`);
      cb.reset(); // Not a failure, just missing
      continue;
    }
