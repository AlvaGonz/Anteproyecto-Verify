...
public class ConsentGateTests
{
    ...
    [Fact]
    public async Task TransUnion_ShouldBeBlocked_WhenNoConsentimientoVigente()
    {
        // Arrange
    ...
    [Fact]
    public async Task TransUnion_ShouldBeBlocked_WhenConsentimientoVersionIsOutdated()
    {
        // Arrange
        var projectId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        ...
        // Assert
        Assert.False(result.IsSuccess);
        Assert.Contains("Se requiere renovar el consentimiento", result.Mensaje);
